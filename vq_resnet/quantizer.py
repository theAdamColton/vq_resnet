...
class VectorQuantize(nn.Module):
    ...
    def decode_embed_ind(self, embed_ind):
        quantize = batched_embedding(embed_ind, self._codebook.embed)
        quantize = self.project_out(quantize)
        if self.accept_image_fmap:
            quantize = rearrange(quantize, "b h w c -> b c h w")
        return quantize

class VectorQuantizeRandPermute(VectorQuantize):
    ...
    def forward(self, x):
        """
        x is [b, c, h, w]
        permutes x along the h and w dimensions
        """
        in_shape = x.shape
        x = rearrange(x, 'b c h w -> b c (h w)')
        rand_indeces = torch.randperm(x.shape[-1])
        x = x[:, :, rand_indeces]
        x = rearrange(x, 'b c (h w) -> b c h w', h=in_shape[2], w=in_shape[3])
...
class ResidualVQ(nn.Module):
    ...
    def forward(
        ...
        return_all_resid = False,
    ):
        """
        returns (quantized_out, all_indices, all_losses, all_resid?, all_codes?)
        """
        ...
        for quantizer_index, layer in enumerate(self.layers):
            ...
            if return_all_resid:
                all_resid.append(residual.detach().clone())
